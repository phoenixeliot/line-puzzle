...
const InteractiveBoard = observer(
  ({ board, style = {} }: { board: Board; style?: CSSProperties }) => {
    ...
    return (
      <>
        ...
        <button onClick={() => board.solveChoicelessMoves()}>
          Solve choiceless moves
        </button>
        <div>
          <code>{board.toString()}</code>
        </div>
        <div>
          <code>{JSON.stringify(board.data)}</code>
        </div>
        <div>
          <code>{board.count}</code>
          <button onClick={() => board.increment()}>+1</button>
        </div>
      </>
    );
  }
